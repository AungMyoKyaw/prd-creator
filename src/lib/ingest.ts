...
function collectFilePaths(structure: unknown): string[] {
  ...
  while (queue.length > 0) {
    ...
    if (typeof current === "object") {
      ...
      if (typeof record.name === "string" && typeof record.type === "string" && record.type === "file") {
...
export function analyzeGitIngest(raw: string): IngestInsight {
  ...
  filePaths.forEach((path) => {
    ...
    const language = extension ? LANGUAGE_BY_EXTENSION[extension] ?? "Other" : "Other";
  ...
  const moduleFrequency = moduleNames.reduce<Map<string, number>>((acc, module) => {
